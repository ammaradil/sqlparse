...
def test_grouping_identifier_list_subquery():
    ...
    p = bplsqlparse.parse("select * from ("
...
def test_grouping_identifier_list_case():
    p = bplsqlparse.parse('a, case when 1 then 2 else 3 end as b, c')[0]
    assert isinstance(p.tokens[0], sql.IdentifierList)
    p = bplsqlparse.parse('(a, case when 1 then 2 else 3 end as b, c)')[0]
    assert isinstance(p.tokens[0].tokens[1], sql.IdentifierList)
...
def test_grouping_identifier_list_other():
    # issue2
    p = bplsqlparse.parse("select *, null, 1, 'foo', bar from mytable, x")[0]
...
def test_grouping_identifier_list_with_inline_comments():
    # issue163
    p = bplsqlparse.parse('foo /* a comment */, bar')[0]
    assert isinstance(p.tokens[0], sql.IdentifierList)
    assert isinstance(p.tokens[0].tokens[0], sql.Identifier)
...
def test_grouping_alias_case():
    # see issue46
    p = bplsqlparse.parse('CASE WHEN 1 THEN 2 ELSE 3 END foo')[0]
...
def test_grouping_subquery_no_parens():
    ...
    p = bplsqlparse.parse('CASE WHEN 1 THEN select 2 where foo = 1 end')[0]
    assert len(p.tokens) == 1
...
def test_grouping_idlist_function():
    # see issue10 too
    p = bplsqlparse.parse('foo(1) x, bar')[0]
    assert isinstance(p.tokens[0], sql.IdentifierList)
...
def test_grouping_function():
    p = bplsqlparse.parse('foo()')[0]
    assert isinstance(p.tokens[0], sql.Function)
    p = bplsqlparse.parse('foo(null, bar)')[0]
    assert isinstance(p.tokens[0], sql.Function)
...
def test_grouping_varchar():
    p = bplsqlparse.parse('"text" Varchar(50) NOT NULL')[0]
    assert isinstance(p.tokens[2], sql.Function)


def test_statement_get_type():
    def f(sql):
        return bplsqlparse.parse(sql)[0]
...
def test_identifier_consumes_ordering():
    # issue89
    p = bplsqlparse.parse('select * from foo order by c1 desc, c2, c3')[0]
...
def test_comparison_with_functions():
    # issue230
    p = bplsqlparse.parse('foo = DATE(bar.baz)')[0]
    assert len(p.tokens) == 1
    assert isinstance(p.tokens[0], sql.Comparison)
    assert len(p.tokens[0].tokens) == 5
    assert p.tokens[0].left.value == 'foo'
    ...
    p = bplsqlparse.parse('DATE(foo.bar) = DATE(bar.baz)')[0]
    assert len(p.tokens) == 1
    assert isinstance(p.tokens[0], sql.Comparison)
    assert len(p.tokens[0].tokens) == 5
    ...
    p = bplsqlparse.parse('DATE(foo.bar) = bar.baz')[0]
    assert len(p.tokens) == 1
    assert isinstance(p.tokens[0], sql.Comparison)
    assert len(p.tokens[0].tokens) == 5
    assert p.tokens[0].left.value == 'DATE(foo.bar)'
...
def test_forloops(start):
    p = bplsqlparse.parse('{0} foo in bar LOOP foobar END LOOP'.format(start))[0]
    assert (len(p.tokens)) == 1
    assert isinstance(p.tokens[0], sql.For)


def test_nested_for():
    p = bplsqlparse.parse('FOR foo LOOP FOR bar LOOP END LOOP END LOOP')[0]
    assert len(p.tokens) == 1
    for1 = p.tokens[0]
...
def test_nested_begin():
    p = bplsqlparse.parse('BEGIN foo BEGIN bar END END')[0]
    assert len(p.tokens) == 1
    outer = p.tokens[0]
...
def test_aliased_literal_without_as():
    p = bplsqlparse.parse('1 foo')[0].tokens
    assert len(p) == 1
