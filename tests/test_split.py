...
def test_split_backslash():
    stmts = bplsqlparse.parse(r"select '\\'; select '\''; select '\\\'';")
    assert len(stmts) == 3
...
def test_split_stream():
    ...
    stmts = bplsqlparse.parsestream(stream)
...
def test_split_encoding_parsestream():
    stream = StringIO("SELECT 1; SELECT 2;")
    stmts = list(bplsqlparse.parsestream(stream))
...
def test_split_simple():
    stmts = bplsqlparse.split('select * from foo; select * from bar;')
    assert len(stmts) == 2
