:mod:`bplsqlparse` -- Parse SQL statements
=======================================

.. module:: bplsqlparse
   :synopsis: Parse SQL statements.

The :mod:`bplsqlparse` module provides the following functions on module-level.

.. autofunction:: bplsqlparse.split

.. autofunction:: bplsqlparse.format

.. autofunction:: bplsqlparse.parse

In most cases there's no need to set the `encoding` parameter. If
`encoding` is not set, bplsqlparse assumes that the given SQL statement
...
The :meth:`~bplsqlparse.format` function accepts the following keyword arguments.
